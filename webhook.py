...
async def post_to_chat_webhook(entries: List[Dict[str, str]], chat_webhook_url: str,
    ...
    """
    Send a report using the GRID layout.
    """
...
async def add_to_pending_chat(entry: Dict[str, str], chat_webhook_url: str, pending_chat_lock,
    ...
    """
    MODIFIED: This now ONLY accumulates entries. It does NOT send them.
    Sending is deferred until send_all_batches is called at the end.
    """
...
async def send_all_batches(chat_webhook_url: str, pending_chat_lock,
                           pending_chat_entries: List, chat_batch_size: int, post_webhook_func):
    """
    NEW FUNCTION: Slices the accumulated data into batches and sends them one by one.
    Call this AFTER post_job_summary to ensure proper order.
    """
    if not chat_webhook_url: return
    
    async with pending_chat_lock:
        all_entries = pending_chat_entries[:]
        pending_chat_entries.clear()
        
    if not all_entries: return
    
    # Process all buffered entries in batches
    batch_count = 1
    for i in range(0, len(all_entries), chat_batch_size):
        batch = all_entries[i : i + chat_batch_size]
        await post_webhook_func(batch, batch_count_override=batch_count)
        batch_count += 1
        # Small delay to ensure order in Google Chat and prevent rate limits
