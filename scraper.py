...
#######################################################################
#                  DATE/TIME RANGE SELECTION HELPERS
#######################################################################
...
def get_date_time_range_from_config() -> dict | None:
...
async def apply_date_time_range(page: Page, store_name: str) -> bool:
...
async def process_single_store(context: BrowserContext, store_info: Dict[str,str], queue: Queue):
    ...
    for attempt in range(WORKER_RETRY_COUNT):
        ...
        try:
            ...
            # Apply date/time range filter if configured
            date_range_applied = await apply_date_time_range(page, store_name)
